      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <header className="bg-white border-b border-gray-200 px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              {sourceFilter && (
                <span className="hidden sm:inline-flex items-center text-xs sm:text-sm text-primary-600 bg-primary-50 px-2 py-0.5 sm:py-1 rounded-full whitespace-nowrap">
                  筛选: {sourceFilter === 'official' ? '官方' : sourceFilter === 'github' ? 'GitHub' : sourceFilter}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={clearChat}
              className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors touch-feedback"
              title="清空对话"
            >
